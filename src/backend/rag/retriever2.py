...
def recuperar_hibrido(pergunta: str, k: int = 10, alpha: float = 0.6) -> list: 
    """
    Combina BM25 e semântico.
    alpha = peso do semântico (0 = só BM25, 1 = só semântico, 0.6 = padrão)
    """

    #NOVAS ALTERAÇÕES (GPT) - LINHAS DE BAIXO
    ...
    #LINHA NOVAS (AS DUAS DE BAIXO) - GPT
    ...
    score = 1/(k_dense+60) + 1/(k_bm25+60)
    idx = np.argsort(score_final)[::-1][:k]

    return [
        {"id": indexer.chunks_globais[i]["id"], "texto": indexer.chunks_globais[i]["texto"], "source": indexer.chunks_globais[i]["source"], "score": float(score_final[i])}
        for i in idx
    ]
